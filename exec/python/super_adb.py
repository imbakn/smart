...
def install():
    smarthome = os.environ["SMARTDIR"]
    out = call('push %s /data/local/tmp/busybox' % (smarthome + '/resource/busybox-armv7l'))
...
def interact():
    ...
    shell.sendline('[ -f  /data/local/tmp/reboot ] && rm /data/local/tmp/reboot')
